...
import { getPatientData, createPatientSymptomLog, getPatientSymptomsData } from './patients.service';
...
export const setPatientSymptoms: RouteHandlerMethod = async (req: FastifyRequest, reply: FastifyReply) => {
  try {
    ...
    const result = await createPatientSymptomLog({...data, authId: req.session.id, patientId: params.patientId });
